...
class Pages extends MX_Controller {
	...
	{	
		...
		$this->load->model('testimonial/testimonial_model','testimonialmodel');
        $results = $this->testimonialmodel->view_testimonial();
		foreach($results as $outputfill)
		{
			$sendthru["description"] = $outputfill['description'];
			$sendthru["author"] = $outputfill['author'];
			$sendthru["location"] = $outputfill['location'];
			$sendthru["createdDate"] = $outputfill['created_date'];
		}
		// loading captcha helper
			...
			$sendthru['captcha'] = create_captcha($vals);
  			$sendthru['image'] = $sendthru['captcha']['image'];
			$this->session->set_userdata('captchaWord',$sendthru['captcha']['word']);
			
			//captcha ends	 
			//$this->view($uri,$output);
