...
class Users extends MX_Controller {
		...
		if (!$this->session->userdata('session_user'))
		{
			...
			{	
				$username = array('field' => 'email', 'label' => 'Email address', 'value' => $this->input->post('email'));
                                $username = array('field' => 'username', 'label' => 'Email address', 'value' => $this->input->post('email'));
	...
	function create_account($redirect = '')
	{
		...
		// loading captcha helper
		    ...
		    'img_url' => base_url().'./static/uploads/captcha/',    
		    'img_width' => '350',
		    'img_height' => '45',
		    );
		      $output['captcha'] = create_captcha($vals);
      			$output['image'] = $output['captcha']['image'];
      			$this->session->set_userdata('captchaWord',$output['captcha']['word']);
		if (isset($_POST['captcha']) && !$this->_captcha_check())
		{
			$this->form_validation->set_error('Please enter correct Captcha words!.');
		}
			//captcha ends
		// require first name?
		...
		$this->form_validation->set_rules('email', 'Email', 'required|valid_email|unique[users.email]|trim|strtolower');
                ...
                $this->form_validation->set_rules('username', 'Username', 'required|unique[users.username]|trim');
		...
		if (count($_POST) && $this->form_validation->run())
		{
			...
			$redirect = $this->core->encode('/users/account');
		...
		$output['form:birthDate'] = set_value('email', $this->input->post('birthDate'));
	...
	function account($redirect = '')
	{
		...
		$this->core->required = array(
			...
			'firstName' => array('label' => 'First Name', 'rules' => 'required'),
			'lastName' => array('label' => 'Last Name', 'rules' => 'required'),
			'city' => array('label' => 'City', 'rules' => 'required'),
			'postcode' => array('label' => 'Post Code', 'rules' => 'required|regex_match[/^[0-9]{9}$/]'),
			'phone' => array('label' => 'Cell Phone', 'rules' => 'required|regex_match[/^[0-9+]{13}$/]'),
			'workPhone' => array('label' => 'Work Phone', 'rules' => 'required|regex_match[/^[0-9+]{13}$/]'),
			
			'confirmPassword' => array('label' => 'Confirm Password', 'rules' => 'matches[password]')
		...
		//$output['form:state'] = set_value('state', $data['state']);
		$output['form:postcode'] = set_value('postcode', $data['postcode']);
		$output['form:phone'] = set_value('phone', $data['phone']);
               // $output['form:workPhone'] = set_value('workPhone', $data['workPhone']);
		$output['select:country'] = @display_countries('country', set_value('country', $data['country']), 'id="country" class="formelement"');
		$output['select:state'] = @display_states('state', set_value('state',$data['state']),'id="state" class="formelement"');		$values = array(
	...
	function _captcha_check()
	{
		...
		{
			$this->form_validation->set_message('captcha_check', 'Please enter correct Captcha words!');
			return FALSE;
		}
	}
