...
class Core {
	...
	function web_form()
	{
		...
		if (!$webform = $this->CI->core->get_web_form_by_ref($this->CI->core->decode($this->CI->input->post('formID'))))
		{
		 ...
		 if ($webform['formName'] == "contactForm") {

			
        
		    $this->CI->form_validation->set_rules('fullName', 'Full name', 'required');
		    $this->CI->form_validation->set_rules('comments', 'Comments', 'required');
		    
		    $this->CI->form_validation->set_rules('phonenumber', 'Phone number', 'required');
		  //  $this->CI->form_validation->set_rules('phoneNumber', 'Phone number', 'required');
